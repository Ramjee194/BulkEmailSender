...
app.post("/send", async (c) => {
  try {
    ...
    // If no user config found, get default config from DB
    if (!userConfig) {
      userConfig = userDatabase.getUserDefaultSMTPConfig(user.id);
    }

    // Fallback to .env settings if still no config
    if (!userConfig && process.env.SMTP_USER) {
      console.log("⚠️ Using system-wide .env SMTP settings as fallback");
      userConfig = {
        id: "env-fallback",
        name: "System Default",
        ...
        from_name: process.env.FROM_NAME || "Bulk Email Sender",
      } as any;
    }
      ...
      return c.json(
        {
          success: false,
          message:
            "No SMTP configuration found. Please add an SMTP configuration in Settings first.",
        },
        400
      );
    }
